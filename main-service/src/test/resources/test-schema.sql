...
CREATE TABLE IF NOT EXISTS event (id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,category_id bigint REFERENCES category(id) ON DELETE CASCADE NOT NULL,initiator_id bigint REFERENCES person(id) ON DELETE CASCADE NOT NULL,location_id bigint REFERENCES location(id) ON DELETE CASCADE NOT NULL,title varchar(120) NOT NULL CHECK(LENGTH(title) >= 3),annotation varchar(2000) NOT NULL CHECK(LENGTH(annotation) >= 20),description varchar(7000) NOT NULL CHECK(LENGTH(description) >= 20),created_on timestamp WITHOUT TIME ZONE NOT NULL,event_date timestamp WITHOUT TIME ZONE NOT NULL,published_on timestamp WITHOUT TIME ZONE NULL,paid boolean DEFAULT false,participant_limit int DEFAULT 0,confirmed_requests int DEFAULT 0,request_moderation boolean DEFAULT true,state varchar(9) NOT NULL,views bigint DEFAULT 0);
CREATE TABLE IF NOT EXISTS compilation (id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,title varchar(50) NOT NULL CHECK(LENGTH(title) >= 1),pinned boolean DEFAULT false);
...
CREATE TABLE IF NOT EXISTS request (id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,event_id bigint REFERENCES event(id) ON DELETE CASCADE NOT NULL,requester_id bigint REFERENCES person(id) ON DELETE CASCADE NOT NULL,created_on timestamp WITHOUT TIME ZONE NOT NULL,status varchar(9) NOT NULL);
-- ==============================================================================
DELETE FROM person;
